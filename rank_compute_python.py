...
def init_rank_result_list():
	for i in range(len(numbers_list)):
		result_list.append(0)


def init_sorted_num_list():
	for i in range(len(numbers_list)):
		sorted_num_list.append(0)
...
def show_sorted_list():
	print("Sorted list based on rank...")
	for i in range(len(numbers_list)):
		sorted_num_list[result_list[i]] = numbers_list[i]

	for i in range(len(sorted_num_list)):	
		logging.info("%s : Number = %s", i, sorted_num_list[i]) 
...
if __name__ == "__main__": 
	...
	read_from_file()
	init_rank_result_list()
	init_sorted_num_list()
	...
	show_result_ranks()
	print("finished rank_compute, rank results displayed......")
	show_sorted_list()
	print("program ends......")